...
#define ADC_SAR_IP_SW_MAJOR_VERSION_C               4
...
#if (ADC_SAR_IP_HAS_CTU && (STD_ON == ADC_SAR_IP_EOCTU_ENABLED) && (STD_ON == ADC_SAR_IP_EOC_ENABLED))
static inline void Adc_Sar_CheckAndCallEoctuNotification(const uint32 Instance);
#endif
...
static inline uint32 Adc_Sar_GetADCLKSELValue(Adc_Sar_Ip_ClockSelType ClockSel)
{
#if ADC_SAR_IP_HAS_CLKSEL_EXTENDED
    ...
    return Temp;
#endif /* ADC_SAR_IP_HAS_CLKSEL_EXTENDED */
}
#endif /* ADC_SAR_IP_HAS_ADCLKSEL */
...
static inline void Adc_Sar_CheckAndCallEoctuNotification(const uint32 Instance)
{
    volatile const uint32 * CIMRBaseAddr;
    volatile uint32 * CEOCFBaseRAddr;
    volatile uint32 * ISRAddr;
    uint32 Imr;  /* Interrupt mask */
    uint32 Cimr; /* Channel Interrupt Mask */
    uint32 Ceocfr; /* Channel Pending Flag*/
    uint32 ChannelValidMask;
    ...
    if (Instance >= ADC_INSTANCE_COUNT)
    {
        ...
        CIMRBaseAddr   = &(CIMR(AdcAEBasePtr,   0U));
...
#endif /* FEATURE_ADC_HAS_APPLICATION_EXTENSION */
    {
        ...
        CIMRBaseAddr   = &(CIMR(AdcBasePtr,   0U));
    ...
    {
        for (GroupIdx = 0U; GroupIdx < Adc_Sar_Ip_au8AdcGroupCount[Instance]; GroupIdx++)
        {
            ChannelValidMask = Adc_Sar_Ip_au32AdcChanBitmap[Instance][GroupIdx];
            /* Read channels interrupt mask and status in this group */
            Cimr             = CIMRBaseAddr[GroupIdx] & ChannelValidMask;
            Ceocfr           = CEOCFBaseRAddr[GroupIdx] & ChannelValidMask;
            /* CPR_RTD_00644: Check if the interrupt is spurious or not (both enabled and flag bits are set) */
            if ((Ceocfr & Cimr) != 0U)
            {
                /* Clears flag by w1c */
                CEOCFBaseRAddr[GroupIdx] = Ceocfr;
                ValidInterrupt = TRUE;
                /* Continue to check and clear pending flags for the rest of channel groups.
                   Because the EOC events of channels might be consecutively issued very fast.
                   For instance:
                     - While ISR handler is processing EOC event of a channel
                     - other channels might complete their conversions and raise the interrupt requests.
                   Therefore, we should clear all the individual channel flags (CEOCFR) along with the CTU flag ISR[EOCTU].
                */
            }
        }
...
#endif /* (ADC_SAR_IP_HAS_CTU && (STD_ON == ADC_SAR_IP_EOCTU_ENABLED) && (STD_ON == ADC_SAR_IP_EOC_ENABLED)) */
                                                          ...
                                                          uint16 VectAdr)
{
    ...
    uint32 Wtimr = 0U;
    uint32 WdgFlags = 0U;
    /* Use 16-bit width instead of 32-bit to avoid CWE INTEGER_OVERFLOW when performing multiplication in macros ADC_SAR_IP_WDG_REG_MASK below */
    ...
    if ((Aworr != 0U) && (Cwenr != 0U))
    {
        ...
        ThresholdIndex = (uint16)(Adc_Sar_Ip_axAdcSarState[Instance].ChanWdgThresholdIndex[ChanIdx]);
        ...
        if (WdgFlags != 0u)
        {
            ...
            *AWORRAddr = Mask;
            ...
            WdgFlags = (uint32)(WdgFlags << (ThresholdIndex << 1U));
            /* Check if the callback is not null */
...
void Adc_Sar_Ip_IRQHandler(const uint32 Instance)
{
    ...
    uint32 Isr = 0u;
    uint32 Wtisr = 0u;
#if ((ADC_SAR_IP_HAS_CTU && (STD_ON == ADC_SAR_IP_EOCTU_ENABLED)) || \
...
#if (ADC_SAR_IP_HAS_CTU && (STD_ON == ADC_SAR_IP_EOCTU_ENABLED) && (STD_ON == ADC_SAR_IP_EOC_ENABLED))
        Adc_Sar_CheckAndCallEoctuNotification(Instance);
#endif /* (ADC_SAR_IP_HAS_CTU && (STD_ON == ADC_SAR_IP_EOCTU_ENABLED) && (STD_ON == ADC_SAR_IP_EOC_ENABLED)) */
...
#if ADC_SAR_IP_HAS_CTU
#if (ADC_SAR_IP_HAS_CTU_TRIGGER_MODE == 0u)
...
Adc_Sar_Ip_StatusType Adc_Sar_Ip_SelfTest(const uint32 u32Instance)
{
    ...
    uint32 AdcClkSel = 0u;
#endif /* (ADC_SAR_IP_SELFTEST_FULL_CLK && ADC_SAR_IP_HAS_ADCLKSEL) */
    volatile uint32 * MCRAddr = NULL_PTR;
    volatile const uint32 * MSRAddr = NULL_PTR;
...
#endif /* ADC_SAR_IP_HAS_SELFTEST_USE_CH32 */
#if ADC_SAR_IP_HAS_SELFTEST_STCR1
    volatile uint32 * STCR1Addr = NULL_PTR;
#endif /* ADC_SAR_IP_HAS_SELFTEST_STCR1 */
    volatile uint32 * STCR2Addr = NULL_PTR;
#if ADC_SAR_IP_HAS_SELFTEST_STCR3
...
#if ADC_SAR_IP_SELFTEST_FULL_CLK
    ...
    if (ADC_SAR_IP_STATUS_SUCCESS == Status)
    {
        ...
        *MCRAddr |= ADC_MCR_ADCLKSEL(1U);
        ...
        if (ADC_SAR_IP_STATUS_SUCCESS == Status)
        {
#endif /* ADC_SAR_IP_SELFTEST_FULL_CLK */
...
#endif /* ADC_SAR_IP_HAS_ADCLKSEL */
        /* Save the current state of ADCKO in order to restore it */
        AdcAutoClkOff = *MCRAddr & ADC_MSR_ACKO_MASK;
        /* Disable AutoClockOff */
        Mcr &= ~(ADC_MCR_ACKO_MASK);
        *MCRAddr = Mcr;
                                            ...
                                            const boolean bAllowRestart)
{
    ...
    {
        *MCRAddr &= ~(ADC_MCR_NSTART(1U));
#if ADC_SAR_IP_ABORTCHAIN_WORKAROUND
        /* Workaround to fix abort chain cannot clear MSR[NSTART] bit in scan mode */
        while (ElapsedTicks < TimeoutTicks)
        {
            ElapsedTicks += OsIf_GetElapsed(&CurrentTicks, ADC_SAR_IP_TIMEOUT_TYPE);
        }
#endif /* ADC_SAR_IP_ABORTCHAIN_WORKAROUND */
    }
...
* Description   : This function configures user gain and offset.
*
* @implements      Adc_Sar_Ip_SetUserGainAndOffset_Activity
* END**************************************************************************/
                                     ...
                                     const uint8 Offset)
{
    ...
    if (Instance >= ADC_INSTANCE_COUNT)
    {
        ...
        RegVal = ADC_AE_USER_OFFSET(Offset);
        RegVal |= ADC_AE_USER_GAIN(Gain);
...
#endif /* FEATURE_ADC_HAS_APPLICATION_EXTENSION */
    {
        ...
        RegVal = ADC_USER_OFFSET(Offset);
        RegVal |= ADC_USER_GAIN(Gain);
                                         ...
                                         const uint16 u16TempSenseAdcConvData)
{
    ...
    uint32 TempValue;
    sint32 Tets;
    const uint16 AdcVoltageRefs[ADC_INSTANCE_COUNT] = ADC_SAR_IP_VOLTAGE_REFERENCE_VAL;
#if (ADC_SAR_IP_SET_RESOLUTION == STD_ON)
    uint8 Resolution;
#else
    uint8 Resolution = ADC_SAR_IP_MAX_RESOLUTION;
    ...
    /* Calculate "Vets" (according to currently configured resolution) */
    Vets = (uint32)((Vref * (uint32)u16TempSenseAdcConvData) >> Resolution);

    /* Calculate "Tca1 * Vets" (using 64-bit type for internal calculation to avoid overflow) */
    TempValue = (uint32)( ((uint64)Adc_Sar_TempsenseConvFp2Int(Tca1) * (uint64)Vets) / (uint64)ADC_SAR_IP_TEMPSENSE_MULTIPLIER );
    ...
    /* Calculate "(Vets ^ 2)" (overflow will not occur because maximum of Vets is: 5 * ADC_SAR_IP_TEMPSENSE_MULTIPLIER = 50,000) */
    Vets = (Vets * Vets) / ADC_SAR_IP_TEMPSENSE_MULTIPLIER;

    /* Calculate "Tca2 * (Vets ^ 2)" (using 64-bit type for internal calculation to avoid overflow) */
    TempValue = (uint32)( ((uint64)Adc_Sar_TempsenseConvFp2Int(Tca2) * (uint64)Vets) / (uint64)ADC_SAR_IP_TEMPSENSE_MULTIPLIER );
