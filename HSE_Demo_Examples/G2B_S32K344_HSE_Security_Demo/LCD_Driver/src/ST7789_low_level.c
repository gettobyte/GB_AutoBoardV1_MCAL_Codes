...
extern const Lpspi_Ip_ExternalDeviceType Lpspi_Ip_DeviceAttributes_SpiExternalDevice_0_Instance_4;

//PTE15
#define gb_ST7789_DC_pin_low()  Siul2_Dio_Ip_WritePin(LCD_DC_PORT, LCD_DC_PIN, 0);
#define gb_ST7789_DC_pin_high() Siul2_Dio_Ip_WritePin(LCD_DC_PORT, LCD_DC_PIN, 1);

//PTB12
#define gb_ST7789_CS_pin_low()  Siul2_Dio_Ip_WritePin(LCD_PCS_PORT, LCD_PCS_PIN, 0);
#define gb_ST7789_CS_pin_high() Siul2_Dio_Ip_WritePin(LCD_PCS_PORT, LCD_PCS_PIN, 1);

//PTA18
#define gb_ST7789_reset_pin_low()  Siul2_Dio_Ip_WritePin(LCD_RST_PORT, LCD_RST_PIN, 0);
#define gb_ST7789_reset_pin_high() Siul2_Dio_Ip_WritePin(LCD_RST_PORT, LCD_RST_PIN, 1);
...
inline void GB_MA_SPI_send_byte_conti(uint8_t *val, uint16_t count, uint32_t timeout) {
	Lpspi_Ip_SyncTransmit(
			&Lpspi_Ip_DeviceAttributes_SpiExternalDevice_0_Instance_4, val,
			&rx_value, count, timeout); //Transfer the data from MOSI to MISO
}

inline void GB_MA_SPI_exchange_byte(uint8_t *rx_value, uint8_t count, uint32_t timeout) {
...
inline void GB_ST7789_SendCommand(uint8_t commandByte, uint8_t *dataBytes,
		uint8_t numDataBytes, uint32_t timeout) {
...
inline void GB_ST7789_SendData(uint8_t *data, uint32_t data_size) {
	gb_ST7789_DC_pin_high();

	GB_MA_SPI_send_byte_conti(data, data_size, 5000);
}

inline void GB_ST7789_SendDataIm(const uint8_t *data, uint32_t data_size) {
	gb_ST7789_DC_pin_high();
...
void GB_ST7789_Init() {
	...
	GB_ST7789_SendCommand(ST77XX_MADCTL, &MADCTL_SetRotation0, 1,
	GB_ST7789_TimeOut);
...
//	uint8_t RowAddr[4] = {0x00,0x00,320>>8,320&0xFF};
	...
	uint8_t d19_data_15[16] = { ILI9341_GMCTRP1, 0x0F, 0x31, 0x2B, 0x0C, 0x0E,
			0x08, 0x4E, 0xF1, 0x37, 0x07, 0x10, 0x03, 0x0E, 0x09, 0x00 };
	GB_ST7789_SendData(d19_data_15, 16);

	uint8_t d20_data_15[16] = { ILI9341_GMCTRN1, 0x00, 0x0E, 0x14, 0x03, 0x11,
...
void ST7789_SetAddressWindow(uint16_t x0, uint16_t y0, uint16_t x1, uint16_t y1) {
	 ...
	 *
	 */
	uint8_t ColAddr[4] = { x_start >> 8, x_start & 0xFF, x_end >> 8, x_end
			& 0xff };
	//uint8_t ColAddr[4]={ 0x00,0x00,0x00, 0xEF};

	GB_ST7789_SendCommand(ST77XX_CASET, ColAddr, 4, GB_ST7789_TimeOut);
	uint8_t RowAddr[4] = { y_start >> 8, y_start & 0xFF, y_end >> 8, y_end
			& 0xFF };
...
void ST7789_WriteStringBox(uint16_t x, uint16_t y, uint16_t x1, uint16_t y1,
...
void ST7789_WriteDec(uint16_t x, uint16_t y, uint32_t gb_val, FontDef font,
		uint16_t color, uint16_t bgcolor) {
...
void ST7789_Float(uint16_t x, uint16_t y, float gb_value, FontDef font,
		uint16_t color, uint16_t bgcolor) {
