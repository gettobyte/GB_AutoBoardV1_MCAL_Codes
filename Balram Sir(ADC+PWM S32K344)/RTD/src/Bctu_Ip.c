...
#define BCTU_IP_AR_RELEASE_MINOR_VERSION_C       4
#define BCTU_IP_AR_RELEASE_REVISION_VERSION_C    0
#define BCTU_IP_SW_MAJOR_VERSION_C               2
...
static uint32 Bctu_Ip_au32BctuFifoDmaChannel[BCTU_IP_FIFO_COUNT];
...
uint32 Bctu_Ip_GetStatusFlags(const uint32 u32Instance)
{
    ...
    Mask |= ((Reg & ((uint32)BCTU_MSR_TRGF_MASK << Index)) != 0U) ? (BCTU_IP_STATUS_TRG << Index) : 0U;
                              ...
                              const uint32 u32Mask)
{
    ...
    ClrMask |= ((u32Mask & (BCTU_IP_STATUS_TRG << Index)) != 0U) ? (BCTU_MSR_TRGF_CLR_MASK << Index) : 0U;
...
#if (BCTU_IP_DEV_ERROR_DETECT == STD_ON)
    DevAssert(u32Instance < BCTU_INSTANCE_COUNT);
    DevAssert(u8AdcIdx < FEATURE_BCTU_NUM_ADC);
#endif /* (BCTU_IP_DEV_ERROR_DETECT == STD_ON) */

    const BCTU_Type * const BctuBasePtr = Bctu_Ip_apxBctuBase[u32Instance];
    Data = (BctuBasePtr->ADCDR[u8AdcIdx] & BCTU_ADCDR_ADC_DATA_MASK) >> BCTU_ADCDR_ADC_DATA_SHIFT;
                           ...
                           const uint8 u8FifoIdx)
{
    ...
    switch (u8FifoIdx)
    {
        case 0u:
            ...
            AdcIdx = (uint8)((ResultFifo & BCTU_FIFO1DR_ADC_NUM_MASK) >> BCTU_FIFO1DR_ADC_NUM_SHIFT);
            Data = (ResultFifo & BCTU_FIFO1DR_ADC_DATA_MASK) >> BCTU_FIFO1DR_ADC_DATA_SHIFT;
            break;
        case 1u:
            ResultFifo = BctuBasePtr->FIFO2DR;
            AdcIdx = (uint8)((ResultFifo & BCTU_FIFO2DR_ADC_NUM_MASK) >> BCTU_FIFO2DR_ADC_NUM_SHIFT);
                           ...
                           Bctu_Ip_FifoResultType * const pResult)
{
    ...
    switch (u8FifoIdx)
    {
        case 0:
            ...
            pResult->AdcNum     = (uint8)  ((ResultFifo & BCTU_FIFO1DR_ADC_NUM_MASK)  >> BCTU_FIFO1DR_ADC_NUM_SHIFT);
            break;
        case 1:
            ResultFifo         = BctuBasePtr->FIFO2DR;
            ...
            pResult->AdcNum     = (uint8)  ((ResultFifo & BCTU_FIFO2DR_ADC_NUM_MASK)  >> BCTU_FIFO2DR_ADC_NUM_SHIFT);
            break;
        default:
#if (BCTU_IP_DEV_ERROR_DETECT == STD_ON)
            DevAssert(FALSE);
                            ...
                            const uint8 FifoIndex)
{
    ...
    (void)Dma_Ip_SetLogicChannelCommand(Bctu_Ip_au32BctuFifoDmaChannel[FifoIndex], DMA_IP_CH_SET_HARDWARE_REQUEST);
}
#endif /* BCTU_IP_DMA_SUPPORTED */
                                                        ...
                                                        const uint8 NumFifoConfigs)
{
    ...
    for (Index = 0U; Index < NumFifoConfigs; Index++)
    {
        ...
        Bctu_Ip_au32BctuFifoDmaChannel[Index] = FifoConfig->FifoDmaChannel;
#endif /* BCTU_IP_DMA_SUPPORTED */
